...
# Function to summarize final summary chunks and only return the final summary
def summarize_final_chunks(summaries):
    summary_text = " ".join(summaries)
    chunks = chunk_text(summary_text, FINAL_SUMMARY_CHUNK_SIZE)
    
    # Process only the last chunk and return it as the final summary
    final_summary = summarize_chunk(chunks[-1])
